...
class Solver {
	...
	void Solve(int l, QMatrix Q, double[] b_, byte[] y_, double[] alpha_,
...
final class Solver_NU extends Solver {
	...
	void Solve(int l, QMatrix Q, double[] b, byte[] y, double[] alpha,
			double Cp, double Cn, double eps, SolutionInfo si, int shrinking) {
		this.si = si;
...
public class svm {
	...
	public static double svm_predict_probability(svm_model model, svm_node[] x,
		...
		if ((model.param.svm_type == svm_parameter.C_SVC || model.param.svm_type == svm_parameter.NU_SVC)
			...
			for (i = 0; i < nr_class; i++)
				for (int j = i + 1; j < nr_class; j++) {
					pairwise_prob[i][j] = Math.min(Math.max(sigmoid_predict(
							dec_values[k], model.probA[k], model.probB[k]),
							min_prob), 1 - min_prob);
