...
import util.SSRFilter;

public class EyeDetectorParaSVM {
	...
	public EyeDetectorParaSVM(svm_model model) {
	...
	public int[] detectAllFaces(Image even_image) {
		 
		final int imgHeight = even_image.getHeight(null);
		final int imgWidth = even_image.getWidth(null);
		setfHeight(imgHeight);
		setfWidth(imgWidth);
		...
		retimg = even_image;
		// Initializing
		pixels = ImageProcessing.extractPixels(even_image, 0, 0, fWidth, fHeight,pixels);
		...
		ii = ImageProcessing.calculateIntegralImage(imgWidth, imgHeight, grayPixels, s, ii);
		faces = 0;
		...
		/*
		if (faces == 0 ){
			SSRFilter filter2 = new SSRFilter(48, 31, ii);
			coords = detectFaces(filter2);
		}
		if (faces == 0 ){
			SSRFilter filter2 = new SSRFilter(55, 27, ii);
			coords = detectFaces(filter2);
		}
		if (faces == 0 ){
			SSRFilter filter2 = new SSRFilter(42, 27, ii);
			coords = detectFaces(filter2);
		}
		if (faces == 0 ){
			SSRFilter filter3 = new SSRFilter(54, 35, ii);
			coords = detectFaces(filter3);
		}
		if (faces == 0 ){
			SSRFilter filter4 = new SSRFilter(38, 25, ii);
			coords = detectFaces(filter4);
		}
		if (faces == 0 ){
			SSRFilter filter4 = new SSRFilter(30, 18, ii);
			coords = detectFaces(filter4);
		}*/
 	...
 	private int[] detectFaces(SSRFilter filter) {
		...
		for (int y = 0; y < fHeight - filter.getHeight(); y++){
			...
			for (int x = 0; x < fWidth - filter.getWidth(); x++) {
				xCor = x + 1 + (filter.getWidth() / 2);			
				if (filter.foundFaceCandidate(x, y)	&& ImageProcessing.isSkinPixel(pixels[yCor * fWidth	+ xCor]))
					//desenhaRoi(Color.RED, x, y, 1, 1);
					label = CC.findClustersLabels(xCor, yCor, fWidth);
					//desenhaRoi(Color.YELLOW, x, y, 1, 1);
			}
		}
		...
		if (label != 0) {
		 ...
		 for (int i=0; i<centers.length; i++){
			 desenhaRoi(Color.LIGHT_GRAY, centers[i][0],  centers[i][1], 5, 5);
		 }
			...
			if (centers != null) {
				...
				if (counter != -1) {
					...
					if (cluster > 1)
						cResults = multiplyByArea(CC.indecies, CC.clusters,
								CC.counters, cResults);
 ...
 public int[] getFace() {
	...
	public Image getRetimg() {
		return retimg;
	}
	private Point findPupil(int x0, int y0, int width, int height,
			int labels[], int pupilsMembers[][], int limit) {
		int label = 0; // binarize the sector and look for the appropriate
						// cluster
		...
		if (label != 0) {
			CC.findRootLabels();
			return CC.findPupilsClustersCenters(x0, y0, CC.clusters, limit,
					width, height, grayPixels, fWidth);
		} else
			return null;
	}
	...
	private int findPupilsCandidates(SSRFilter filter, int centers[][]) {
																...
																// clusters
		{
			...
			lp1 = findPupil(x - halfW, y - halfH, width, height, labels,
					pupilsMembers, limit);
			if (lp1 == null)
				continue;

			// find right pupil in sector s3
			rp1 = findPupil(x + sixthW, y - halfH, width, height, labels,
					pupilsMembers, limit);
			if (rp1 == null)
				continue;
	...
	private svm_node[] extractEyesTemplate(int x0, int y0, int x1, int y1) {
		...
		oY = y0 - (int) nY;
		sX = oX;
		sY = oY; // rotate the template to a horizontal position
		for (int y = 0; y < 21; y++) {
			cX = sX;
			cY = sY;
			for (int x = 0; x < 35; x++) {
	...
	private Vector loadEyesTemplates(int counter) {
		...
		for (int i = 0; i <= counter; i++) {
			template = extractEyesTemplate(eyes[i][0], eyes[i][1], eyes[i][2],
					eyes[i][3]);
			desenhaRoi(Color.BLUE, eyes[i][0],  eyes[i][1], 5,5);
			desenhaRoi(Color.BLUE, eyes[i][2],  eyes[i][3], 5,5);
	...
	private double[] classify(Vector templates) {
		//System.out.println("classify: " + templates);
		double cResults[] = new double[templates.size()];
		double[] decValues = new double[1];

		for (int i = 0; i < templates.size(); i++) {
			svm.svm_predict_values(model, (svm_node[]) templates.get(i),decValues);
			cResults[i] = decValues[0] * model.label[0];
		}
		return cResults;
	}
 
	private void desenhaRoi(Color c, int x, int y, int w, int h) {
		BufferedImage buffer = new BufferedImage(retimg.getWidth(null), retimg
				.getHeight(null), BufferedImage.TYPE_INT_RGB);
		Graphics g = buffer.createGraphics();
		g.drawImage(retimg, 0, 0, retimg.getWidth(null),
				retimg.getHeight(null), null);
		g.setColor(c);
		g.drawRect(x, y, w, h);
		retimg = new ImageIcon(buffer).getImage();
	}

 	// ////////////////////////////////////////////////

	private double[] multiplyByArea(int indecies[], int clusters[],
			int counters[], double cResults[]) {
	...
	private int[] findBestEyesTemplate(double cResults[]) {
		...
		if (max > 0) {
			...
			face[2] = eyes[index][2];
			face[3] = eyes[index][3];
			desenhaRoi(Color.BLACK, face[0], face[1], face[2] - face[0],face[3] - face[1]);
			return face;
		} else
			return null;
	}
	...
	private Point findNoseTip(int face[]) {
		...
		Point candidates[] = findNoseBridge(length, ii); // find nose bridge
		// candidates
		if (candidates != null) {
			int gradiants[] = calculateGradiants(candidates, length);
			int uMin1 = Integer.MAX_VALUE, uMin2 = Integer.MAX_VALUE, lMin1 = Integer.MAX_VALUE, lMin2 = Integer.MAX_VALUE;
			...
			for (int i = start; i <= minIndex; i++)
				// after finding the smallest gradiant ( nose trills ) find the
				// largest
				if ((gradiants[i] > max) && (gradiants[i] != Integer.MAX_VALUE)) // gradiant
				// above
				// it
				// (
				// nose
			...
			double angle = Math.atan(slope); // rotate ROI to it's original
			// state
			double x = Math.cos(angle) * noseTip.getX() - Math.sin(angle)
					* noseTip.getY();
			double y = Math.sin(angle) * noseTip.getX() + Math.cos(angle)
					* noseTip.getY();
			x += face[0];
			y += face[1];
