...
public class FaceDetector {
	...
	public int[] detectAllFaces(Image image) {
		retimg = image;
		// Initializing
		pixels = ImageProcessing.extractPixels(image, 0, 0, fWidth, fHeight,
				pixels);
		...
		ii = ImageProcessing.calculateIntegralImage(fWidth, fHeight,
				grayPixels, s, ii);
		faces = 0;
		int coords[];
		...
		// If no faces were found with the previous filter apply a smaller one,
		// and so on...
		// if (faces == 0)
		{
	...
	private int[] detectFaces(SSRFilter filter) {
		...
		ConnectedComponents CC = new ConnectedComponents(labels,
				clustersMembers);

		for (int y = 0; y < fHeight - filter.getHeight(); y++)
			for (int x = 0; x < fWidth - filter.getWidth(); x++) {
				xCor = x + 1 + (filter.getWidth() / 2); // +1 : transition error
														// in sectors
														// calculation
				yCor = y + 1 + (filter.getHeight() / 2);
				if (filter.foundFaceCandidate(x, y)
						&& ImageProcessing.isSkinPixel(pixels[yCor * fWidth
								+ xCor]))
		...
		if (label != 0) {
			...
			int centers[][] = CC.processClusters(filter.getArea() / 48d,
					fWidth, fHeight);
	...
	private svm_node[] extractEyesTemplate(int x0, int y0, int x1, int y1) {
		...
		nY = 8 * scale; //(yLen != 0 ? (8 * yInc * xLen) / yLen : 8 * scale);
		oY = y0 - (int) nY;
		sX = oX;
	...
	private double[] classify(Vector templates) {
		...
		for (int i = 0; i < templates.size(); i++) {
			if (i==0)
				continue;
			for (svm_node s: (svm_node[]) templates.get(i)){
				System.out.print(s.index+" ");
			}
			System.out.println("");
			svm.svm_predict_values(model, (svm_node[]) templates.get(i),
					decValues);
			cResults[i] = decValues[0] * model.label[0];
			
			for (int j =0; j<cResults.length; j++){
				System.out.println("cResults: " + cResults[j]);
			}
			return cResults;
		}
		return cResults;
