...
public class ImageProcessing {
	...
	public static int[][] calculateIntegralImage(int width, int height,
			int grayPixels[], int s[][], int ii[][]) {
	...
	public static boolean isSkinPixel(int color) {
		...
		a = r + (g / 2);
		b = (sqrt3 / 2d) * g;
		aValue = (int) Math.round(a / 0.01d);
		bValue = (int) Math.round(b / 0.01d);
		if ((aValue >= 49) && (aValue <= 59) && (bValue >= 24)
				&& (bValue <= 29)) // thresholds calculated from analyzing
			return true; // 566685 skin samples
		else
			return false;
	}
	...
	public static int[] toBinary(int grayPixels[], int binaryPixels[],
			int threshold) {
	...
	public static BufferedImage toInvertedBinary(int grayPixels[],
